"""Card top 5 (fila) × 2 octavos de hoy (columna): su pronóstico. En empates, quién pasa entre paréntesis. Pillow -> PNG."""
import os
...
MATCHES = [('ARG – EGI','HOY 18:00'), ('SUI – COL','HOY 22:00')]
# (pos, nombre, pts, color, [(pred, quien_pasa|None), ...])
TOP = [
  (1,'Jan Alemany',126,GOLD,[('2-0',None),('0-1',None)]),
  (2,'Javi Albácar',122,SILVER,[('2-1',None),('1-0',None)]),
  (3,'Tomás Brillas',121,BRONZE,[('2-0',None),('1-1','SUI')]),
  (4,'Nacho de Oza',119,PRIZE,[('2-0',None),('1-1','SUI')]),
  (5,'Gonzalo Gramunt',117,PRIZE,[('2-0',None),('1-1','COL')]),
]
CX = [258, 340]  # centros de las 2 columnas
...
for i,(pos,name,pts,col,preds) in enumerate(TOP):
    ...
    for (pred,adv),cx in zip(preds,CX):
        if adv:
            tx(cx,y+15,pred,16,WHITE,True,anchor='mm')
            tx(cx,y+32,'pasa '+adv,8,BLUEL,anchor='mm')
        else:
...
out=os.path.join(DESK,'podio_octavos_07jul.png'); img.save(out); print('->',out)
