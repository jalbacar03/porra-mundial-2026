"""Cupón del 2º partido (Corea del Sur - Chequia) con 1X2 + TODOS los marcadores. -> PNG."""
import os, cairosvg
...
SCORES = [('1-1',59),('1-2',16),('2-1',15),('2-2',9),('0-0',4),('2-0',4),
          ('0-1',3),('0-2',3),('1-0',3),('3-1',2),('1-3',1)]
...
p.append('<text x="24" y="56" font-family="Helvetica, Arial, sans-serif" font-size="11" fill="#8a8f99">2º partido · Grupo A · 119 predicciones</text>')
...
p.append('<text x="24" y="108" font-family="Helvetica, Arial, sans-serif" font-size="18" font-weight="700" fill="#ffffff">Corea del Sur <tspan fill="#5f636e" font-size="14">vs</tspan> Chequia</text>')
p.append('<text x="24" y="127" font-family="Helvetica, Arial, sans-serif" font-size="11" fill="#8a8f99">Mañana, 04:00 h · Guadalajara</text>')

# 1X2
boxes = [('1 · COREA','20%','24 votos',False),('X · EMPATE','61%','72 votos',True),('2 · CHEQUIA','19%','23 votos',False)]
xs = [24,138,252]
for (label,pct,votes,lead),x in zip(boxes,xs):
    ...
    p.append(f'<text x="{x+52}" y="161" font-family="Helvetica, Arial, sans-serif" font-size="10" font-weight="700" fill="{sub}" text-anchor="middle">{label}</text>')
...
for i,(score,n) in enumerate(SCORES):
    ...
    p.append(f'<text x="24" y="{top+15}" font-family="Helvetica, Arial, sans-serif" font-size="13" font-weight="700" fill="{col}">{score}</text>')
    p.append(f'<rect x="78" y="{top+7}" width="180" height="9" rx="4.5" fill="#2c303a"/>')
    p.append(f'<rect x="78" y="{top+7}" width="{bar}" height="9" rx="4.5" fill="{barcol}"/>')
    p.append(f'<text x="300" y="{top+15}" font-family="Helvetica, Arial, sans-serif" font-size="11" fill="#8a8f99" text-anchor="end">{pct}%</text>')
    p.append(f'<text x="356" y="{top+15}" font-family="Helvetica, Arial, sans-serif" font-size="13" font-weight="700" fill="{col}" text-anchor="end">{n}</text>')

p.append(f'<text x="190" y="{H-12}" font-family="Helvetica, Arial, sans-serif" font-size="10" fill="#5f636e" text-anchor="middle">porra-mundial-2026 · 11 marcadores distintos · 1 de cada 2 ve empate</text>')
p.append('</svg>')

out = os.path.join(DESK, 'partido2_corea_chequia.png')
