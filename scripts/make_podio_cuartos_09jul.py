"""Card top 5 × el cuarto de hoy (1 columna): su pronóstico. Empate → quién pasa entre paréntesis. Pillow -> PNG."""
import os
...
MATCHES = [('FRA – MAR','HOY 22:00')]
# (pos, nombre, pts, color, [(pred, quien_pasa|None)])
TOP = [
  (1,'Jan Alemany',127,GOLD,[('1-0',None)]),
  (2,'Javi Albácar',125,SILVER,[('2-2','FRA')]),
  (3,'Tomás Brillas',124,BRONZE,[('2-1',None)]),
  (4,'Nacho de Oza',122,PRIZE,[('2-0',None)]),
  (5,'Gonzalo Gramunt',119,PRIZE,[('2-0',None)]),
]
CX = [312]  # centro de la única columna
...
tx(24,48,'Top 5 · su pronóstico para el cuarto de hoy',10.5,MUT)
tx(W-24,32,'TOP 5',13,GOLD,True,anchor='rm')
# Cabecera de columna (partido + hora)
for (lab,hora),cx in zip(MATCHES,CX):
    tx(cx,100,lab,10.5,BLUEL,True,anchor='mm'); tx(cx,114,hora,8,DIM,anchor='mm')
...
for i,(pos,name,pts,col,preds) in enumerate(TOP):
    ...
    nm=name
    while tw(nm,15,True) > 200 and len(nm) > 4: nm=nm[:-1]
    tx(62,y+11,nm if nm==name else nm.rstrip()+'…',15,WHITE,True)
    tx(62,y+31,str(pts)+' pts',10,MUT)
    for (pred,adv),cx in zip(preds,CX):
        if adv:
            tx(cx,y+15,pred,17,WHITE,True,anchor='mm')
            tx(cx,y+33,'pasa '+adv,8.5,BLUEL,anchor='mm')
        else:
            tx(cx,y+20,pred,18,WHITE,True,anchor='mm')
    if i<len(TOP)-1:
        d.line([(20*S,(y+RH-6)*S),((W-20)*S,(y+RH-6)*S)], fill='#23262f', width=1*S)
tx(W/2,H-13,'porra-mundial-2026 · cuartos de hoy',9,DIM,anchor='ma')
out=os.path.join(DESK,'podio_cuartos_09jul.png'); img.save(out); print('->',out)
