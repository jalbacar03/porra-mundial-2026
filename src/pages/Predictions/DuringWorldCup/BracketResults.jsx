...
import { KNOCKOUT_PREDICTIONS_DEADLINE, KNOCKOUT_ROUND_DEADLINE_OVERRIDES } from '../../../hooks/useCountdown'
...
export default function BracketResults({ session }) {
  ...
  const roundDeadline = useMemo(() => {
    ...
    STAGE_ORDER.forEach(stage => {
      ...
      // Override manual de ampliación (p.ej. octavos hasta dom 21:00) — manda
      // sobre el cálculo automático "primer partido − 1 min".
      if (KNOCKOUT_ROUND_DEADLINE_OVERRIDES[stage]) map[stage] = KNOCKOUT_ROUND_DEADLINE_OVERRIDES[stage]
    })
    return map
  }, [matches])
            ...
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: '12px' }}>
              <div style={{ minWidth: 0 }}>
                <div style={{ fontSize: '11px', fontWeight: '700', color: 'var(--text-primary)' }}>
                  Cierran los {activeRoundData.matches.filter(m => m.status !== 'finished').length} {activeRoundData.label.toLowerCase()} que faltan a la vez
                </div>
